...
export const createHintBanner = async (notebookPanel, pioneer, cell, cellIndex, promptGroup, prompt, uuid, preReflection, hintType, requestId, options) => {
    var _a;
    ...
    (_a = notebookPanel.content.node.parentElement) === null || _a === void 0 ? void 0 : _a.insertBefore(hintBanner, notebookPanel.content.node);
    // Mark banner active to allow global UI styling (disable hint buttons)
    setBannerActive(true);
    const renderRatingUI = (contentToShow) => {
        ...
        const hintBannerButtonClicked = async (evaluation) => {
            ...
            try {
                ...
                const idx = meta.findIndex((m) => String(m.requestId) === String(requestId));
                if (idx >= 0) {
        ...
        unhelpfulButton.onclick = () => {
            ...
            escalateButton.onclick = async () => {
                ...
                try {
                    const response = await requestAPI('ta', {
                        method: 'POST',
                        body: JSON.stringify({
                    ...
                    if (response.statusCode !== 200) {
                        showDialog({
                            title: (response === null || response === void 0 ? void 0 : response.message) || 'Error',
                            buttons: [
                                Dialog.createButton({
                                    label: 'Dismiss',
    ...
    if (options === null || options === void 0 ? void 0 : options.skipNetwork) {
    ...
    const hintRequestCompleted = (hintContent, requestId) => {
        const hintHistory = cell.getMetadata('hintHistory') || [];
        cell.setMetadata('hintHistory', [
            ...hintHistory,
            {
    ...
    const hintRequestCancelled = (requestId) => {
        hintBanner.remove();
        hintBannerPlaceholder.remove();
        setBannerActive(false);
        showDialog({
    ...
    const hintRequestError = (e) => {
        ...
        try {
            ...
            if (container) {
                ...
                if (span)
                    span.innerHTML = String(remainingHints[hintType]);
            }
        }
        catch (err) {
        ...
        pioneer.exporters.forEach(exporter => {
            pioneer.publishEvent(notebookPanel, {
                ...
                eventInfo: {
                    gradeId: gradeId,
                    requestId: e === null || e === void 0 ? void 0 : e.message,
                    promptGroup: promptGroup,
                    prompt: prompt,
                    uuid: uuid,
                    preReflection: preReflection,
    ...
    try {
        const response = await requestAPI('reflection', {
            method: 'POST',
            body: JSON.stringify({
                request_id: requestId,
        ...
        if (!response) {
            ...
            const intervalId = setInterval(async () => {
                const response = await requestAPI('check', {
                    method: 'POST',
                    body: JSON.stringify({
                        request_id: requestId
                    })
