...
const activateHintBot = async (
  ...
  console.log('[HintBot] Number of cells:', cells?.length || 0);
  ...
  if (notebookPanel.model.getMetadata('firstTimeUsingHintbot') === undefined) {
    try {
      ...
      const everResp = await requestAPI<{ student_id: string; ever_requested: boolean }>('has_ever_requested');
  ...
  if (cells) {
    ...
    for (let i = 0; i < cells.length; i++) {
      ...
      const gradeId: string | undefined = nbgrader?.grade_id;
