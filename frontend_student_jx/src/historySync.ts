...
// Data shape similar to frontend_student HistoricHintItem
type HistoricItem = {
  id: number;
  ...
  instructor_helpful?: boolean | null;
};

async function fetchAllForProblem(problem_id: string): Promise<HistoricItem[]> {
  const [ai, fb] = await Promise.all([
    requestAPI<any[]>(`query_all_hint?problem_id=${encodeURIComponent(problem_id)}`),
    requestAPI<any[]>(`query_all_feedback?problem_id=${encodeURIComponent(problem_id)}`),
  ]);
  const aiItems: HistoricItem[] = (ai || []).map((a: any) => ({
      id: a.request_id || a.id,
      ai_request_id: a.request_id || a.id,
      type: 'ai',
      subtype: (a.hint_type || a.type) as any,
  ...
  const fbItems: HistoricItem[] = (fb || []).map((f: any) => ({
...
function pickEarliestUnrated(items: HistoricItem[]): HistoricItem | null {
  const unrated = items
...
export async function syncHistoryAndMaybeShowRatingBanner(
  ...
  for (let i = 0; i < cells.length; i++) {
    ...
    const hintHistory = items.map((it) => ({
      ...
      requestId: it.type === 'ai' ? it.id : (it.ai_request_id ?? it.id),
    ...
    if (earliest && !document.getElementById('hint-banner')) {
      if (earliest.type === 'ai') {
        ...
        await createHintBanner(
          ...
          hintType,
          String(earliest.id),
          { preloadedContent: earliest.content, skipNetwork: true }
        );
      } else {
        // Instructor feedback banner with rating only
        const banner = document.createElement('div');
        banner.id = 'hint-banner';
        c && notebookPanel.content.node.parentElement?.insertBefore(banner, notebookPanel.content.node);
        // Mark banner active to allow global UI styling (disable hint buttons)
        try { (await import('./bannerActive')).setBannerActive(true); } catch { }
        banner.innerHTML = '';
        // Title row
        ...
        const saveFeedbackRating = async (val: boolean) => {
        ...
        const refreshHistoryBar = async () => {
          ...
          const idx = meta.findIndex((m: any) => String(m.instructorRequestId) === String(earliest.id));
          ...
          if (idx >= 0) {
            ...
            meta[idx].instructor_helpful = meta[idx].instructor_helpful ?? null;
        ...
        helpful.onclick = async () => {
          ...
          try {
            ...
            if (idx >= 0) { meta[idx].instructor_helpful = true; c.setMetadata('hintHistory', meta); }
            await refreshHistoryBar();
          } finally { try { (await import('./bannerActive')).setBannerActive(false); } catch { } banner.remove(); }
        };
        unhelpful.onclick = async () => {
          ...
          try {
            ...
            if (idx >= 0) { meta[idx].instructor_helpful = false; c.setMetadata('hintHistory', meta); }
            await refreshHistoryBar();
          } finally { try { (await import('./bannerActive')).setBannerActive(false); } catch { } banner.remove(); }
        };
      }
