...
def program_to_essential_tokens(program: str, strip_chars="\n\r\t\f ") -> List[str]:
    ...
    for component in lex_output:
        ...
        if token_type == pygments.token.Whitespace and token_value == "\n":
            is_start_of_line = True
            if (
                len(simplified_program_tokens) == 0
                or simplified_program_tokens[-1] != "\n"
            ):
                simplified_program_tokens.append(token_value)
        elif (
            not is_start_of_line
            and token_type == pygments.token.Text
            and re.match(r"^\s+$", token_value)
        ):
    ...
    while (
        len(simplified_program_tokens) > 0
        and simplified_program_tokens[-1].strip(strip_chars) == ""
    ):
